...
4. Transformer.ipynb - this file contains the implementation of the vision transformer.
5. VGG.ipynb - this file contains the code for using the VGG model.
6. VGG19_crop.ipynb - here you will find the code for using the VGG19 on the images with the applied central crop.
7. VGG19_transformer.ipynb - this file contains the implementation of the vision transformer added in tandem with the first 4 convolutional layers of VGG19.
...
For the training data, we utilized the following dataset preprocessing techniques. We re-scaled the numeric values of pixels to lie between 0 and 1. Pixel value rescaling was done to reduce the computational complexity for both forward and back propagation and to reduce the risk of overflow. As for data augmentation, we applied random rotation between 0 and 45 degrees, horizontal, and vertical flips, shifted the image along X and Y axis by 30%, and finally, used zoom-in and zoom-out methods. The main reason for the data augmentation is to make our training dataset as rich as possible.

## Utilized models

We then proceeded to train multiple neural networks. First, we trained a custom network with multiple convolutional layers as the baseline. This model doesn’t use any pre-trained network and has 2.1 million trainable parameters. The architecture of the baseline network consists of 4 repeated blocks depicted in Figure 1. 

![Convolutional block](figures/CNN.png "Convolutional block")

Figure 1. Custom CNN block.

The output of the 4 CNN blocks is flattened to a single vector and then interpreted by the multilayered perceptron with 4 dense layers depicted in Figure 2. MLP accepts the flattened feature vector learned by the preceding CNN blocks and then makes the decision on whether the input image contains malignant tumor in the central patch. 

![Perceptron block](figures/MLP.png "Perceptron block")

Figure 2. The multilayer perceptron block.

Apart from the custom CNN baseline, we utilized transfer learning and constructed 3 more networks which use VGG16 (14.8 million parameters), VGG19 (20.0 million parameters), InceptionV3 (21.8 million parameters) and ResNet-50 (23.5 million parameters) as base models initialized with weights from the ImageNet database. All layers of the pre-trained models were frozen. All 3 networks based on the pre-trained models take the output of the pre-trained models (excluding their fully connected layers), flatten it to a vector and feed it to the MLP in Figure 2. We chose to use the same MLP architecture for the upper prediction layer across all tested architectures to make them more comparable.

We now discuss the transformer architecture and its applicability to vision. Multi-head attention allows the transformer to attend to multiple regions of the input sequence at once and thus compute a more complete learned representation of the input. On a higher level, transformer consists of 2 key blocks: transformer encoder, and transformer decoder. Transformer encoder produces a representation of the input sequence by applying multi-head self-attention. Meanwhile, the decoder generates the result by first applying the multi-head self-attention to the output of the previous decoder layer, and then applying encoder-decoder attention which uses encoder final state output as keys and values, and the output of the previous decoder as queries. Despite its merits, transformer model is typically applied to sequential data, such as text, and not images. To make images compatible with the transformer architecture, we patched the 32*32 centrally cropped image into 196 patches, 5*5 pixels each. This procedure along with the model architecture are demonstrated below.

![Image patching](figures/Patching.png "Image patching")

Figure 3. Image pre-processing for the transformer encoder-decoder architecture. The image is first center-cropped to produce 32*32 fragment. The fragment is then patched into 5 * 5 pixel segments and the segments are reshaped into a sequence. Here we use an image from the PCam dataset.
...
Further, we developed a hybrid architecture which combines the benefits of convolutional neural networks for extracting useful features of the data and the benefits of transformer which can learn powerful representations of the input sequence thanks to the multi-head self-attention mechanism. To do this, we first center-cropped the image and then fed the it into VGG19. We then extracted the output of the fourth convolutional layer of VGG19. The extracted feature maps were then reshaped into 512 patches, each 2*2 pixels. Next, the patches were flattened into a single vector and fed into the transformer encoder-decoder as is shown in the diagram below.

![Tandem transformer](figures/Tandem_transformer.png "Tandem transformer")

Figure 5. Transformer applied in tandem with the convolutional layers of VGG19. The feature map from the fourth convolutional layer is patched and reformed into a sequence, which is then fed into the transformer encoder-decoder
...
As evident from the results, central cropping drastically improves the performance of VGG19, likely because it allows the model to focus only on the key central patch which is the only part of the image used for the malignancy label. 

Next, we trained the vision transformer. Further, we wanted to compare the transformer with the hybrid transformer + CNN architecture. Based on the results from VGG19, we applied central cropping to the original image in both cases to maximize the chance of higher accuracy. Both models were trained for 50 epochs on the training set with pre-processed 8000 images in batches of 64 using the Adam optimizer with the learning rate 1e-4 and the categorical cross entropy as the loss function. The validation score is based on the validation set with 1000 images and the final accuracy is evaluated on the test set with another 1000 images. The results of training are presented in Table 3.
